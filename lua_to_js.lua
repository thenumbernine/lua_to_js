...
ast._function.tostringmethods.c = function(self)
	if self.name then
		return 'function '..self.name..'('..table(self.args):mapi(function(arg) 
			return tostring(arg) 
		end):concat', '..') {\n' .. tabblock(self) .. tab() .. '}'
	else
		error("not supporting lambdas")
		-- lambdas?
...
local code = template(path'test.lua':read())
print('original:')
print(code)
